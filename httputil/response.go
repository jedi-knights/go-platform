...
// Encoding happens into a buffer before any headers are sent, so when
// encoding fails the client receives a 500 rather than a 200 with a
// truncated body.
func WriteJSON(w http.ResponseWriter, status int, v any) {
	...
	if _, err := buf.WriteTo(w); err != nil {
		// Headers are already on the wire; surface the transport failure to
		// logs but the response itself cannot be revised.
		slog.Error("failed to write response body", "error", err)
	}
}

// appErrorHTTPStatus maps every predeclared apperrors code to its HTTP status.
// Every code in the apperrors package must appear here; the default branch in
// HTTPStatus is a safety net for unknown future codes, not a substitute for
// known ones.
...
// WriteError writes a JSON error response derived from err. The HTTP status
// is determined by [HTTPStatus]. For non-AppError values, a generic message
// is returned to prevent internal details (SQL errors, file paths, etc.)
// from leaking to clients. WriteError must not be called with a nil error.
