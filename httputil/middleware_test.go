...
// TestLoggingMiddleware_NoWrite confirms that a handler that calls neither
// Write nor WriteHeader does not cause a panic and does not log status 0.
...
// Write (without an explicit WriteHeader) before panicking, the recovery
// must not attempt a second response — the implicitly committed 200 must be
// preserved.
...
func TestRecoveryMiddleware_PanicAfterWriteHeader(t *testing.T) {
	...
	// The handler committed 200 before panicking; recovery must not overwrite it.
	if w.Code != http.StatusOK {
