...
//
// The embedded testing.TB must be initialized with the real *testing.T from
// the enclosing test — never left as nil. Any un-overridden method on a nil
// interface field panics at runtime rather than producing a test failure.
//
// Note: Fatalf sets a flag but does NOT call runtime.Goexit. Code that runs
// after a Fatalf call inside the function under test will still execute.
// This is intentional for spy purposes; it means spyT is unsuitable for
// testing helpers that rely on Fatalf being terminal.
type spyT struct {
	testing.TB
	fataled bool
	errored bool
...
func (*typedNilErr) Error() string { return "typed nil error" }
...
func TestRequireNoError_TypedNilDoesNotFatal(t *testing.T) {
	...
	// A typed nil (*typedNilErr)(nil) assigned to an error interface produces
	// a non-nil interface value, so a naive err != nil check would call Fatal.
...
func TestAssertEqual(t *testing.T) {
	...
	tests := []struct {
		...
		// reflect.DeepEqual distinguishes nil slices from empty slices, so
		// this call intentionally produces an error. See AssertEqual godoc.
...
func TestNewTestLogger(t *testing.T) {
	...
	l.DebugContext(ctx, "debug", "k", "v")
	l.InfoContext(ctx, "info", "k", "v")
	l.WarnContext(ctx, "warn", "k", "v")
	l.ErrorContext(ctx, "error", "k", "v")
	...
	// Enabled must return false so callers that gate expensive arg
	// construction on it skip the work entirely.
	if l.Enabled(ctx, slog.LevelError) {
		t.Error("noop NewTestLogger should report Enabled=false at every level")
	}
}
